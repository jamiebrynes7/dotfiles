...
# Resolve version. Codex releases are tagged `rust-v<version>`.
if [[ $# -ge 1 ]]; then
  VERSION="$1"
else
...
if ! curl -sfI "https://github.com/${REPO}/releases/tag/rust-v${VERSION}" >/dev/null; then
  echo "Error: Release rust-v${VERSION} not found at github.com/${REPO}" >&2
  exit 1
fi
